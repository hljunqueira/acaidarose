...
import { reorderProducts } from '@/lib/repositories/productsRepository'
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { items, collection = 'containers', tenantId } = body

    if (!Array.isArray(items) || items.length === 0) {
      return NextResponse.json({ error: 'Lista de ordenação inválida' }, { status: 400 })
    }

    const success = await reorderProducts(items, collection, tenantId || user?.tenantId)
    if (!success) {
    ...
    console.error('Erro na rota PUT /api/products/reorder:', err)

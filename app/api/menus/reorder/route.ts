...
import { reorderMenus } from '@/lib/repositories/categoriesRepository'
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const success = await reorderMenus(items, tenantId || user?.tenantId)
    if (!success) {
    ...
    console.error('Erro na rota PUT /api/menus/reorder:', err)
    return NextResponse.json({ error: err.message || 'Erro ao reordenar menus' }, { status: 500 })
  }
}

...
if(isset($_REQUEST['position']) && isset($positions[$_REQUEST['position']])) {
    ...
    if ($this->widget_index != '_new_widget' && $addChatway && !$isChatwayAdded) {
...
<li data-id="<?php echo esc_attr($social['slug']) ?>" class="chaty-channel group <?php echo esc_attr($chtPosition) ?> <?php echo ($is_agent == 1) ? "has-agent-view" : "" ?>" data-channel="<?php echo esc_attr($channel_type) ?>" id="chaty-social-<?php echo esc_attr($social['slug']) ?>">
    ...
    <div class="channels-selected__item <?php echo esc_attr(($this->is_pro()) ? 'pro' : 'free'); ?> 1 available">
        <div class="chaty-default-settings flex gap-3">
            <div class="flex relative">
                ...
                <div class="icon chaty-icon icon-md active <?php echo esc_attr($imgClass) ?>" data-title="<?php esc_html_e("Chatway", "chaty"); ?>" id="chaty_image_<?php echo esc_attr($social['slug']) ?>">
    ...
    <!-- remove button start -->
    <button type="button" class="btn-cancel is-chatway" data-social="<?php echo esc_attr($social['slug']); ?>">

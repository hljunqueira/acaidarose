...
export default function QRCodeConfigView({ tenantId }: QRCodeConfigViewProps) {
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      ...
      <div className="bg-white rounded-3xl border border-purple-100 p-6 shadow-xs space-y-6">
        {/* Link de Divulgação */}
        <div className="space-y-1.5">
          <Label className="text-xs font-bold text-foreground">Link de Divulgação Oficial do Cardápio</Label>
          <div className="flex gap-2">
            <Input
              value={publicUrl}
              readOnly
              className="bg-purple-50/50 rounded-xl text-xs font-mono text-purple-950 h-10 select-all"
            />
            <Button
              type="button"
              onClick={handleCopyLink}
              className="bg-purple-700 hover:bg-purple-800 text-white text-xs font-bold px-4 h-10 rounded-xl"
            >
              Copiar Link
            </Button>
          </div>
        ...
        <div className="space-y-1.5 pt-4 border-t border-purple-50">
          <Label className="text-xs font-bold text-foreground">Modo de Utilização do QR Code</Label>
          <select
            value={mode}
            onChange={(e) => setMode(e.target.value as any)}
            className="w-full h-11 px-3.5 rounded-xl border border-input bg-background text-xs font-bold text-foreground focus:outline-none focus:ring-2 focus:ring-purple-600"
          >
        ...
        <div className="space-y-2 pt-4 border-t border-purple-50">
          <Label className="text-xs font-bold text-foreground">Identificação da Mesa no QR Code</Label>
          <div className="space-y-2 text-xs">
            <label className="flex items-center gap-2.5 cursor-pointer">
              ...
              />
              <span className="font-semibold text-foreground">Leitura de QR Code de Mesa Fixo (Placa numerada na mesa)</span>
            </label>
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input
                type="radio"
                ...
                className="text-purple-600 focus:ring-purple-600"
              />
              <span className="font-semibold text-foreground">Mesa digitada pelo cliente no telemóvel</span>
            </label>
          </div>
        </div>

        {/* Switches de Operação */}
        <div className="pt-4 border-t border-purple-50 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs font-bold text-foreground">Utiliza Delivery</div>
              <div className="text-[11px] text-muted-foreground">Permite pedidos para entrega com endereço</div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs font-bold text-foreground">Ativa Pagamento com MB Way no QR Code</div>
              <div className="text-[11px] text-muted-foreground">O cliente pode pagar direto pelo telemóvel ao fechar a comanda</div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs font-bold text-foreground">Habilitar Telefone Internacional</div>
              <div className="text-[11px] text-muted-foreground">Permite números de telemóvel estrangeiros para turistas em Portugal</div>
            </div>
        ...
        <div className="pt-4 border-t border-purple-50 space-y-4">
          <div className="text-xs font-black uppercase text-purple-950 tracking-wider">
            Identificação do Cliente no Envio
          </div>

          {/* Nome */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <span className="text-xs font-bold text-foreground">Nome do Cliente:</span>
              ...
              {(['NONE', 'OPTIONAL', 'REQUIRED'] as const).map((r) => (
                <label key={r} className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    ...
                    className="text-purple-600 focus:ring-purple-600"
                  />
                  <span>{r === 'NONE' ? 'Não pedir' : r === 'OPTIONAL' ? 'Opcional' : 'Obrigatório'}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          {/* Telemóvel */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <span className="text-xs font-bold text-foreground">Telemóvel do Cliente:</span>
            <div className="flex gap-4 text-xs">
              {(['NONE', 'OPTIONAL', 'REQUIRED'] as const).map((r) => (
                <label key={r} className="flex items-center gap-1.5 cursor-pointer">
                  <input
          ...
          {/* NIF */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <span className="text-xs font-bold text-foreground">NIF / Contribuinte do Cliente:</span>
            <div className="flex gap-4 text-xs">
              {(['NONE', 'OPTIONAL', 'REQUIRED'] as const).map((r) => (
                <label key={r} className="flex items-center gap-1.5 cursor-pointer">
                  <input

...
export async function syncAllStoresCatalog(): Promise<{ success: boolean; totalStores: number }> {
  const store = getMockStore()
  // Limpa overrides conflitantes ou garante que todas as lojas tenham acesso aos itens master
  store.tenants.forEach((t) => {
    if (!store.storeProductOverrides[t.id]) {
      store.storeProductOverrides[t.id] = {}
    }
  })
  return { success: true, totalStores: store.tenants.length }
}

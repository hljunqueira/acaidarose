...
export async function createOrder(payload: any): Promise<Order> {
  ...
  try {
    // 1. Inserção na tabela principal orders (o trigger trigger_set_order_daily_seq gera daily_order_seq)
    ...
    // 2. Inserção dos itens na tabela order_items
    ...
    console.error('Erro ao gravar pedido no PostgreSQL:', err)
  }
  ...
  const fallbackOrder: Order = {
    ...
    isTableOrder: payload.isTableOrder !== false,
    items: payload.items || [],
    createdAt: new Date().toISOString(),
  }
  store.orders.unshift(fallbackOrder)

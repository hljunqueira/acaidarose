...
export async function getAuditLogs(options?: {
  ...
  try {
    ...
    return (res.rows || []).map((row: any) => {
      ...
      const tenantName = row.tenant_name || (row.tenant_id?.startsWith('11111111') ? 'Loja 1 - Figueira da Foz (Matriz)' : row.tenant_id?.startsWith('22222222') ? 'Loja 2 - Torres Novas' : row.tenant_id?.startsWith('33333333') ? 'Loja 3 - Aveiro' : 'Franqueadora Master')
